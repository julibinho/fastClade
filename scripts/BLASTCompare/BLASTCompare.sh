...
### new
comparaison_error_log="BLASTCompare.log"
...
if [ "$concat_fastas_check" == true ] && [ "$pssm_input_check" == true ] && [ "$output_check" == true ] && [ "$integer_check" == true ] ; 
	...
	start=$SECONDS
	### the 2>> $comparaison_error_log is new
	for i in "$4"/* ; do psiblast -db "$6".db -in_pssm $i -outfmt $8 ; done >> $6 2>> $comparaison_error_log
	duration=$(( SECONDS - start ))	
