...
if [ "$list_input_check" == true ] && [ "$fastas_input_check" == true ] && [ "$output_check" == true ] && [ "$integer_check" == true ] ; 
	...
	if [ "$output_check" != true ] && [ "$#" != "0" ] && [ "$1" != "-help" ] ; then echo "the output is incorect, a file has maybe already that name, type \"-help\" for more informations" ; fi 
...
The fourth argument specifie at which column should the script read the fastas name in the fastas file (if the document isn't well formated, you can try to input 0 ) 

Typical usage of arguments :
