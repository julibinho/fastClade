...
def exploitRes(SeedDB,SeqDB,result,outputTrue,outputFalse):
    ...
    temp1=(TP/(expected))*100.0
    temp2=(FP/(expected))*100.0
    temp3=(FN/(expected))*100.0
    temp4=((( 2* ((TP)/(TP+FN)) * ((TP)/(TP+FP))) /(((TP)/(TP+FN))+((TP)/(TP+FP)))))*100

    print(str(round(temp1,2))+"%")
    print(str(round(temp2,2))+"%")
    print(str(round(temp3,2))+"%")
    print(str(round(temp4,2))+"%")
