...
int main(int argc, char* argv[]){
	...
	if(myDb.is_open()){
		...
		std::unordered_map<std::string, std::set<std::uint32_t> > testProteinsShingles = shingling_documents_comparisons(argv[2]);
		std::unordered_map<std::string, std::vector<long> > testProteins = minHashSignaturesComparisons(testProteinsShingles,coeffA,coeffB);
