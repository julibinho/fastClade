...
std::unordered_map<std::string,std::pair<std::string,std::string> > getDistributionsMinHash(std::unordered_map<std::string, std::unordered_map<std::string, std::vector<long> > > trainSet, std::unordered_map<std::string, std::vector<long> > testSet){
   ...
   */
   ...
   std::unordered_map<std::string, std::vector<long> >::iterator itTest;
   ...
   //for member of the testSet
   for(itTest = testSet.begin(); itTest != testSet.end(); itTest++){
     //Reset of our variables
     float bestComparison = -1;
     std::string bestFamily = "";
     std::string bestProtein = "";

     //for each family of the trainSet
     for(itTrain = trainSet.begin(); itTrain != trainSet.end(); itTrain++){
       std::string family = (*itTrain).first;

       //for each member of that family in the trainSet
       for(itMember = trainSet[family].begin(); itMember != trainSet[family].end(); itMember++){
          // Take the minHash distance and update informations if needed
          float dist = minHashDistance((*itTest).second,(*itMember).second);
          if(dist > bestComparison){
            bestComparison = dist;
            bestFamily = family;
            bestProtein = (*itMember).first;
          }
       }
     }
     std::pair<std::string,std::string> resultComp;
     resultComp = std::make_pair(bestFamily,bestProtein);
     result.insert(std::pair<std::string,std::pair<std::string,std::string> >((*itTest).first, resultComp));
   }
