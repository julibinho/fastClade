...
int writeResultDistribution(std::unordered_map<std::string,std::pair<std::pair<std::string,float>,std::pair<std::string,float> > > dataComparisons,std::string pathToWrite){
  /**
   * Write in a file the result obtain by the comparison function
   */

  std::ofstream myFile(pathToWrite);
  std::unordered_map<std::string,std::pair<std::pair<std::string,float>,std::pair<std::string,float> > >::iterator it;
  std::string name;
  std::string bestBet;
  ...
  if(myFile.is_open()){
    for(it = dataComparisons.begin(); it != dataComparisons.end(); it++){
      name = (*it).first;
      similIntraFamily = (*it).second.first.second;
      similExtraFamily = (*it).second.second.second;
      ...
      myFile << name << "\t" << bestBet << "\t" << similIntraFamily << "\t" << similExtraFamily << "\n";
    }
  }
